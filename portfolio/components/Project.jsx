    ...
    <div id="Project" className="h-fit target-section">
      ...
      >
        <TypewriterEffectSmooth
          className={` ${window.outerWidth>450&&window.outerWidth<640?"mb-8":""} w-fit text-white font-bold inter-var ${
...
<div className="flex flex-wrap relative bottom-20 justify-evenly">
      {project.map((proj, index) => (
                  <CardContainer className="inter-var w-11/12 mx-auto sm:w-fit bg-slate-200 shadow-white ">
        ...
        <div key={index} className="h-fit w-11/12 px-2 mx-auto rounded-lg shadow-white p-2">
          <section   className=" h-fit relative p-2"
  style={{
    backgroundSize: "400% 400%",
    backgroundPosition: "99.9995% 50%"
  }}>
            <video ref={el => videoRefs.current[index] = el} className=" w-full object-fill h-full rounded-md">
              <source src={proj.video} type="video/mp4" />
            </video>
          </section>
            <hr />
          <div className=" h-fit text-white space-y-6 ">
            <div className="w-full h-fit text-2xl  sm:text-4xl mt-3 font-bold playwrite">{proj.title}</div>
            <div className="w-full h-fit text-lg  sm:text-lg text-wrap text-justify">{proj.dec}</div>
          ...
          <div className="w-full h-fit flex justify-center items-center gap-12 py-3 mt-2 bg-slate-300/85 text-black">
              <a href={proj.code} target="_blank" className="font-semibold"><CodeIcon className="mr-2 "/>Code</a>
              <a href="" target="_blank" className=" font-semibold"> <YouTubeIcon  className="mr-2 "/>Demo Video</a>
            </div>
        </div>
        </CardItem>
